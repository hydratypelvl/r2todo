...
export default function Home() {
  ...
  const [selectedDate, setSelectedDate] = useState<string>("2024-07-13") // Default date
  ...
  async function fetchData() {
    ...
    try {
      ...
      const formattedData: Job[] = Object.entries(groupedData).flatMap(([iorder, { time, cars, brands, types, phones }]) =>
        cars.map((car, index) => ({
          time: index === 0 ? time : "",
          car,
  ...
  return (
    <div className="container mx-auto my-4">
      ...
      <div className="flex flex-row justify-between gap-4 my-2 text-center">
        ...
        <h2 className="text-xl font-bold mx-4">{queueRange[0] === 1 ? "Ulbroka" : "Kalnciems"}</h2>
        <Button variant="outline" size="sm" onClick={() => setQueueRange(queueRange[0] === 1 ? [4, 5] : [1, 3])}>
          Mainīt servisu
        </Button>
      </div>
